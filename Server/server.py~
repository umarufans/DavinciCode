...
#牌组current_card：{'username':[pai],'username':[pai]]
	...
	def open(self,username):
		def begin():
			...
			print '开始发牌'
			self.write_message(json.dumps(current_card)) # cu = {'username':[[pai]]}
			print current_card
...
#-------------------抓牌----------------------------
		...
		if msg[1]==1:
			...
			for rest in rest_card:
				...
				if odd and even:
					break
			self.write_message(json.dumps({'odd':odd,'even':even}))
			if (odd and msg[2]==1) or (even and msg[2]==0):
				for player in players:
					if player[1] and username==player[0].username:
						...
						self.write_message(json.dumps(temp_card)) #将这张牌返回给客户端
						break
			else:
